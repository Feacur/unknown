...
void * rhi_memory_heap(void * ptr, size_t size, size_t align) {
	...
	void * ret = os_memory_global_heap(ptr, size);
...
struct RHI_Swapchain rhi_swapchain_init(VkSurfaceKHR surface, VkSwapchainKHR old_swapchain) {
	...
	ret.frames = os_memory_global_heap(NULL, sizeof(*ret.frames) * images_count);
...
void rhi_swapchain_free(struct RHI_Swapchain swapchain) {
	...
	os_memory_global_heap(swapchain.frames, 0);
	...
	VkPipelineShaderStageCreateInfo const shader_stages[] = {
		{
			...
			.module = rhi_shader_module_create("data/shader_vert.spirv"),
			.pName = "main",
		},
		{
			.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO,
			.stage = VK_SHADER_STAGE_FRAGMENT_BIT,
			.module = rhi_shader_module_create("data/shader_frag.spirv"),
			.pName = "main",
		},
	};
...
void rhi_material_init(void) {
	...
	fl_rhi_ud.material.handles = os_memory_global_heap(NULL, sizeof(*fl_rhi_ud.material.handles) * fl_rhi_swapchain.frames_count);
	...
	fl_rhi_ud.material.map = os_memory_global_heap(NULL, sizeof(*fl_rhi_ud.material.map) * fl_rhi_swapchain.frames_count);
	for (uint32_t i = 0; i < fl_rhi_swapchain.frames_count; i++)
...
void rhi_material_free(void) {
	...
	os_memory_global_heap(fl_rhi_ud.material.map, 0);
	...
	os_memory_global_heap(fl_rhi_ud.material.handles, 0);
}
