...
struct GFX {
	...
	VkCommandBuffer command_buffers[GFX_VK_FRAMES_IN_FLIGHT];
...
void gfx_buffer_copy(VkBuffer source, VkBuffer target, VkDeviceSize size) {
	...
	vkQueueSubmit(fl_gfx.device.graphics_queue, 1, &(VkSubmitInfo){
		.sType = VK_STRUCTURE_TYPE_SUBMIT_INFO,
		.commandBufferCount = 1,
		.pCommandBuffers = &command_buffer,
	}, VK_NULL_HANDLE);

	vkQueueWaitIdle(fl_gfx.device.graphics_queue);
...
void gfx_command_pool_init(void) {
	...
	vkAllocateCommandBuffers(
		...
		&(VkCommandBufferAllocateInfo){
			...
			.commandBufferCount = GFX_VK_FRAMES_IN_FLIGHT,
		},
