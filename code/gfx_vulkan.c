...
#define GFX_VK_DEFINE_PROC(name) PFN_ ## name name = gfx_get_instance_proc(#name)
...
AssertStatic(GFX_VK_FRAMES_IN_FLIGHT >= 1);
...
bool gfx_match_sets(
	...
	for (uint32_t i = 0; i < requested_count; i++) {
		...
		available = false;
		DbgPrint("requested value \"%s\" is not available\n", requested);
		found:;
	}
	return available;
}
...
void * gfx_memory_heap(void * ptr, size_t size, size_t align) {
	...
	Assert(align <= UINT16_MAX, "alignment overflow %zu / %u\n", align, UINT16_MAX);
...
struct GFX {
	...
	// synchronization
	VkSemaphore image_available_semaphores[GFX_VK_FRAMES_IN_FLIGHT];
	VkSemaphore render_finished_semaphores[GFX_VK_FRAMES_IN_FLIGHT];
	VkFence     in_flight_fences[GFX_VK_FRAMES_IN_FLIGHT];
	// main command pool
	VkCommandPool   main_command_pool;
	VkCommandBuffer main_command_buffers[GFX_VK_FRAMES_IN_FLIGHT];
...
void * gfx_get_instance_proc(char const * name) {
	void * ret = vkGetInstanceProcAddr(fl_gfx.instance, name);
	Assert(ret, "[gfx] `vkGetInstanceProcAddr(0x%p, \"%s\")` failed\n", fl_gfx.instance, name);
	return ret;
}
...
void gfx_instance_init(void) {
	...
	#if GFX_ENABLE_DEBUG
	GFX_VK_DEFINE_PROC(vkCreateDebugUtilsMessengerEXT);
...
void gfx_instance_free(void) {
	#if GFX_ENABLE_DEBUG
	GFX_VK_DEFINE_PROC(vkDestroyDebugUtilsMessengerEXT);
...
void gfx_device_init(void) {
	...
	for (uint32_t i = 0; i < all_physical_devices_count; i++) {
		...
		if (match)
			...
			DbgPrint("can't satisfy requested extensions set for 0x%p\n", handle);
	}
	...
	vkCreateDevice(
		...
		&(VkDeviceCreateInfo){
			...
			// queue families
			.queueCreateInfoCount = queue_families.count,
...
void gfx_synchronization_init(void) {
	for (uint32_t i = 0; i < GFX_VK_FRAMES_IN_FLIGHT; i++)
		vkCreateSemaphore(
			fl_gfx.device.handle,
...
void gfx_command_pool_init(void) {
	gfx_command_pool_create(fl_gfx.device.physical.main_qfamily_index, GFX_VK_FRAMES_IN_FLIGHT, &fl_gfx.main_command_pool, fl_gfx.main_command_buffers);
...
void gfx_swapchain_init(VkSwapchainKHR old_swapchain) {
	...
	vkCreateSwapchainKHR(
		...
		&(VkSwapchainCreateInfoKHR){
			...
			.queueFamilyIndexCount = queue_families.count,
...
uint32_t gfx_physical_memory_find_type(uint32_t bits, VkMemoryPropertyFlags flags) {
	...
	Assert(false, "can't find memory property with bits %#b and flags %#b\n", bits, flags);
	return 0;
}
...
void gfx_model_init(void) {
	...
	mem_copy(fl_gfx_vertices, target,                           sizeof(fl_gfx_vertices));
	mem_copy(fl_gfx_indices,  target + sizeof(fl_gfx_vertices), sizeof(fl_gfx_indices));
