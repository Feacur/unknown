...
void gfx_init(void) {
	...
	for (uint32_t i = 0; i < requested_instance_extensions_count; i++) {
		...
		Assert(false, "extension \"%s\" is not available\n", requested);
	...
	for (uint32_t i = 0; i < ArrayCount(requested_instance_layers); i++) {
		...
		Assert(false, "layer \"%s\" is not available\n", requested);
	...
	for (uint32_t i = 0; i < ArrayCount(requested_device_extensions); i++) {
		...
		Assert(false, "device extension \"%s\" is not available\n", requested);
...
void gfx_tick(void) {
	...
	fl_gfx.frame = (fl_gfx.frame + 1) % GFX_VK_FRAMES_IN_FLIGHT;
}
