...
LONG os_vectored_exception_handler(EXCEPTION_POINTERS * ExceptionInfo) {
	...
	switch (code) {
		...
		default:
			Assert(!(flags & EXCEPTION_NONCONTINUABLE), "[os] exception 0x%lx\n", code);
			break;
	}
...
void os_sleep(u64 nanos) {
	u32 const millis = nanos / (u64)1000000;
	if (millis == 0)
		YieldProcessor();
	else
		Sleep(millis);
...
u64 os_file_read(struct OS_File const * file, u64 offset_min, u64 offset_max, void * buffer) {
	...
	for (u64 offset = offset_min; offset < offset_max; (void)0) {
		...
		DWORD received_size;
		DWORD const requested_size = min_u64(offset_max - offset, chunk_limit);
...
u64 os_timer_get_nanos(void) {
	...
	return mul_div_u64(elapsed, AsNanos(1), fl_os.timer_frequency.QuadPart);
}
...
void * os_memory_heap(void * ptr, size_t size) {
	if (size > 0)
	{
		if (ptr == NULL)
		{
			...
			Assert(ret != NULL, "[os] `HeapAlloc(%zu)` failed\n", size);
			return ret;
		}
		void * ret = HeapReAlloc(fl_os.heap, HEAP_GENERATE_EXCEPTIONS | HEAP_ZERO_MEMORY, ptr, size);
		Assert(ret != NULL, "[os] `HeapReAlloc(0x%p, %zu)` failed\n", ptr, size);
		return ret ? ret : ptr;
	}
	if (ptr != NULL)
	{
		BOOL const ok = HeapFree(fl_os.heap, 0, ptr);
		Assert(ok == TRUE, "[os] `HeapFree(0x%p)` failed\n", ptr);
		return ok ? NULL : ptr;
	}
	return NULL;
...
void * os_memory_reserve(size_t size) {
	...
	Assert(ret != NULL, "[os] `VirtualAlloc(NULL, %zu, MEM_RESERVE)` failed\n", size);
	return ret;
...
void os_memory_release(void * ptr, size_t size) {
	...
	Assert(ok == TRUE, "[os] `VirtualFree(0x%p, 0, MEM_RELEASE)` failed\n", ptr);
...
void os_memory_commit(void * ptr, size_t size) {
	...
	Assert(mem != NULL, "[os] `VirtualAlloc(0x%p, %zu, MEM_COMMIT)` failed\n", ptr, size);
	// https://learn.microsoft.com/windows/win32/api/memoryapi/nf-memoryapi-virtualalloc
}
...
void os_memory_decommit(void * ptr, size_t size) {
	...
	Assert(ok == TRUE, "[os] `VirtualFree(0x%p, %zu, MEM_DECOMMIT)` failed\n", ptr, size);
	// https://learn.microsoft.com/windows/win32/api/memoryapi/nf-memoryapi-virtualfree
}
...
void * os_shared_load(char * name) {
	...
	Assert(ret != NULL, "[os] `LoadLibraryA(\"%s\")` failed\n", name);
	return ret;
}

void os_shared_drop(void * inst) {
	BOOL const ok = FreeLibrary(inst);
	Assert(ok == TRUE, "[os] `FreeLibrary(\"0x%p\")` failed\n", inst);
}

void * os_shared_find(void * inst, char * name) {
	void * ret = GetProcAddress(inst, name);
	Assert(ret != NULL, "[os] `GetProcAddress(0x%p, \"%s\")` failed\n", inst, name);
	return ret;
}
