...
:build_clang
	...
	(
		...
		%VTXC% "%data%/shader.glsl" -o data/shader.vert.spv
		%FRGC% "%data%/shader.glsl" -o data/shader.frag.spv
	) | pause > nul

	rem link
	echo.[link unknown.exe] %time%
	%LD% ^
		"%temp%/unknown.o"    "%temp%/base.o" ^
		"%temp%/os_windows.o" "%temp%/windows_main.res" ^
